...
const InfiniteTweetList = ({tweets, isError, isLoading, fetchNewTweets, hasMore = false}: InfiniteTweetListProps) => {
...
function TweetCard({id, user, content, createdAt, likeCount, likedByMe}: Tweet) {
  ...
  const toggleLike = api.tweet.toggleLike.useMutation({onSuccess: ({addedLike}) => {
    ...
    trpcUtils.tweet.infiniteFeed.setInfiniteData({onlyFollowingTweets: true}, updateData);
...
function HeartButton({onClick, isLoading, likedByMe, likeCount}: HeartButtonProps) {
  ...
  return (
    <button disabled={isLoading} onClick={onClick} className={`group items-center gap-1 self-start flex transition-colors duration-200 ml-2 ${likedByMe ? "text-red-500" : "text-gray-500 hover:text-red-500 focus-visible:text-red-500"}`}>
      <IconHoverEffect red>
